...
resource "azurerm_storage_account_network_rules" "container_app" {
  ...
  virtual_network_subnet_ids = local.launch_in_vnet ? local.existing_container_app_environment.name == "" ? concat([azurerm_subnet.container_apps_infra_subnet[0].id], local.storage_account_vnet_subnet_allow_list) : concat([data.azurerm_container_app_environment.existing_container_app_environment[0].infrastructure_subnet_id], local.storage_account_vnet_subnet_allow_list) : null
