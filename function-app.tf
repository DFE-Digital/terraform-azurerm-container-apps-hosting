...
resource "azurerm_monitor_diagnostic_setting" "function_apps" {
  ...
  name                       = "${(each.key == "health-api" ? azurerm_linux_function_app.health_api[each.key].name : azurerm_linux_function_app.function_apps[each.key].name)}-diagnostics"
  target_resource_id         = each.key == "health-api" ? azurerm_linux_function_app.health_api[each.key].id : azurerm_linux_function_app.function_apps[each.key].id
